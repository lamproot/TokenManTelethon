...
<div class="container">
    <div class="row">
        <div class="col-sm-12 text-right lang-box">
            <a href="__APP__/Index/dashboard/<?php echo ($code); ?>?lang=ch" class="lang">中文</a>
        </div>
        ...
        <?php if($activity_status == -1): ?><div class="col-sm-12 text-center">
                <a href="<?php echo ($group_activity['join_button_url']); ?>" class="join border-radius"><?php echo ($group_activity['en_join_button_text']); ?></a>
            </div>
        ...
        <?php else: ?>
            <div class="col-sm-12 text-center">
                <p class="desc">Complete the folliwing steps, then get  <?php echo ($group_activity['rate']); ?> FREE <?php echo ($group_activity['rate_unit']); ?> Tokens!</p>
            </div>
            <div class="col-sm-12 text-left">
                <p class="text-left">1. Click to join the SSS Telegram group. After the event, we will verify <?php echo ($group_activity['rate_unit']); ?> offical telegram group, only the effective members in the group will finally get the rewards.</p>
                <a href="<?php echo ($group_activity['join_button_url']); ?>" class="join border-radius text-nowrap">Join the  <?php echo ($group_activity['rate_unit']); ?> Telegram group!</a>
            </div>
            <div class="col-sm-12 text-center">
                <p class="text-left">2. [Importan!] Copy and send the following referral code to the <?php echo ($group_activity['rate_unit']); ?> group in Telegram</p>
                <div class="input-box">
                    <input class="address border-radius text-center" value="/<?php echo ($chat_bot['code_cmd']); echo ($code); ?>"/>
                    <a href="javascript:void(0);" class="copy text-center copy-btn" data-clipboard-action="copy" data-clipboard-target=".address">COPY</a>
                </div>
            </div>
            <div class="col-sm-12 text-left">
                <p class="text-left">3. Share the following link to your friends, each successful recommendation to join this group can get more <?php echo ($group_activity['rate']); ?> <?php echo ($group_activity['rate_unit']); ?>！</p>
                <div class="input-box">
                    <input class="submit border-radius" value="http://xxxxxx/?<?php echo ($chat_bot['code_cmd']); ?>=<?php echo ($code); ?>"/>
                    <a href="javascript:void(0);"  class="copy text-center copy-btn" data-clipboard-action="copy" data-clipboard-target=".submit">COPY</a>
                </div>
            </div><?php endif; ?>
        ...
        <div class="col-sm-6 text-center">
            <div class="data-box">
                <p>Earned <?php echo ($group_activity['rate_unit']); ?></p>
                <h3><?php echo ($code_rate); ?></h3>
            </div>
        </div>
        <div class="col-sm-12 text-center link-box">
           <a class="link" href="<?php echo ($group_activity['bottom_text_url']); ?>"><?php echo ($group_activity['en_bottom_text']); ?></a>
        </div>
    </div>
</div>
